...
// FIX: Guard against null — on slow Android the DOM may not be ready
// when script executes. Re-query elements at click time as fallback.
function _getBtn(id) { return document.getElementById(id); }

(function _wireButtons() {
  const cb = _getBtn('createBtn') || createBtn;
  const jb = _getBtn('joinBtn')   || joinBtn;
  const ri = _getBtn('roomId')    || roomInput;

  if (cb) {
    cb.onclick = () => {
      requestNotifPermission();
      const id = Math.random().toString(36).slice(2, 8).toUpperCase();
      const inp = _getBtn('roomId') || roomInput;
      if (inp) inp.value = id;
      joinRoom(id, "create");
    };
  }
  ...
  if (jb) {
    jb.onclick = () => {
      requestNotifPermission();
      const inp = _getBtn('roomId') || roomInput;
      const r = inp ? inp.value.trim() : '';
      if (r) joinRoom(r, "join");
    };
  }

  // If buttons weren't in DOM yet, retry once after DOMContentLoaded
  if (!cb || !jb) {
  ...
  try {
    {
      ...
      // ── FIX: lazy-load fflate if SW served cached page without it ────────────
      if (meta.zipBundle && meta.zipFiles) {
        if (typeof fflate === "undefined") {
          // fflate not yet loaded — inject CDN script and retry once loaded
          setStatus("📦 Loading extractor…");
          const _s = document.createElement("script");
          _s.src = "https://cdn.jsdelivr.net/npm/fflate@0.8.2/umd/index.js";
          _s.onload = () => {
            // Re-trigger the exact same blob processing with fflate now available
            dlog("[ZIP] fflate lazy-loaded — retrying extraction");
            window._pendingZipBlob = { blob, meta, url };
            window.dispatchEvent(new CustomEvent("tranzo:zip-ready"));
          };
          _s.onerror = () => {
            setStatus("⚠️ Could not load extractor — zip downloaded as-is");
